...
docker-tag:
	...
	docker build -t goodnight:`git describe --always --tags` -t goodnight:latest --build-arg GIT_TAG=`git describe --always --tags` --build-arg GIT_HASH=`git rev-list --max-count=1 --no-merges --abbrev-commit HEAD` .
